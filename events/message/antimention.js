...
exports.run = async (message, client) =>{
    if (message.mentions.members.size !== 0) {
        ...
        if(!message.member.hasPermission("MANAGE_ROLES")){
            ...
            if (message.mentions.members.filter(mentionned => mentionned.roles.cache.filter(role => role.name === "🔇Ne pas mentionner🔇")).size !== 0) {
                log.log("[JeuxGate : Eantimention] Triggered gid" + message.guild.id + message.guild.name + " uid" + message.author.id + " u mentionned" + message.mentions.members.filter(mentionned => mentionned.roles.cache.filter(role => role.name === "🔇Ne pas mentionner🔇")).size + message.mentions.members.filter(mentionned => mentionned.roles.cache.filter(role => role.name === "🔇Ne pas mentionner🔇")).first().displayName + message.mentions.members.filter(mentionned => mentionned.roles.cache.filter(role => role.name === "🔇Ne pas mentionner🔇")).first().roles, "antimention"  , message.guild.id + "/auto" )
                ...
                fs.writeFileSync("./config/guild/" + message.guild.id + "/temp muted/" + message.author.id, message.mentions.members.filter(mentionned => mentionned.roles.cache.filter(role => role.name === "🔇Ne pas mentionner🔇")).first().id.toString())
                ...
                const mentionnopembed = new Discord.MessageEmbed()
                    ...
                    .addField("message :", message.content)
                    .addField('​', '​')
                    .addField(message.mentions.members.filter(mentionned => mentionned.roles.cache.filter(role => role.name === "🔇Ne pas mentionner🔇")).first().displayName + " Si tu penses qu'il ne devrait pas être mute", "tape `jg/unmute @" + message.member.displayName + "` et il sera demute !")
                    .addField('​', '​')
